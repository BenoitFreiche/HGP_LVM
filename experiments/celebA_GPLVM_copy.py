...
#%%
# idx = np.random.randint(0,399,10)

visages = gp.predict_f(X[idx])[0]
plt.figure()
k = 1
for i,j in enumerate(idx):
    plt.subplot(4,5,k)
    plt.imshow(visages[i].numpy().reshape(218,178))
    plt.title('individu {0}'.format(j))
    plt.xticks([])
    plt.yticks([])
    plt.colorbar()
    k+=1
...
k = 1
for j in range(5):
    to_predict = np.asarray([np.zeros(10)]*5)
    for i in range(5):
        to_predict[i][j] = (i-2)*np.std(X[:,j])
    visages = gp.predict_f(to_predict)[0]
    for vis in  visages:
        plt.subplot(5,5,k)
        plt.imshow(vis.numpy().reshape(218,178),cmap = 'gray')
        plt.xticks([])
        plt.yticks([])
        k+=1
plt.show()
